...
resourcename="tp"  # lower case, only
resourcegroup="tp_rg"
...
# git clone https://github.com/apilogicserver/tp.git
# cd tp
...
az group create --name tp_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group tp_rg --sku S1 --is-linux
az container create --resource-group tp_rg --name tp --image apilogicserver/tp:latest --dns-name-label tp --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//tp.westus.azurecontainer.io:5656'
